    ...
    <el-dialog title="登录" :visible.sync="dialogFormVisible" :append-to-body="true">
        <form>
            <div class="form-group">
                ...
                <input type="email" class="form-control" id="username" autocomplete="off" v-model="username">
            </div>
            <div class="form-group">
                <label for="password">密码</label>
                <input type="password" class="form-control" id="password" v-model="password" @keyup.enter="check">
            </div>
...
export default {
    ...
    methods: {
        ...
        async asyncCheck() {
            ...
            let ret1 = await this.$axios.post(
                "/api/login",
